...
// GPU TEXTURES
// ─────────────────────────────────────────────────────────────────────────────
// Engine-specific texture names. Because both the `js` and `v8` engine objects
// instantiate (and run top-level eval) in Max 9, two jit.gl.texture objects would
// otherwise try to bind the SAME name to the shared SIGNe ob3d context, which
// fails ("name already in use. ob3d does not allow multiple bindings") and breaks
// rendering. Giving each engine distinct texture names avoids the collision: the
// dormant engine creates its (never-populated) textures harmlessly, and the active
// engine binds ITS names to the mesh dynamically once build_atlases completes
// (see the `texture` emit at the end of build_atlases). This is the SpiderMonkey
// (js) build, so it uses the _JS suffix; the V8 build uses _V8.
var SYMBOL_ATLAS_NAME  = "SymbolAtlas_JS";
var PATTERN_ATLAS_NAME = "PatternAtlas_JS";

var texSymbols = new JitterObject("jit.gl.texture", "SIGNe");
texSymbols.name = SYMBOL_ATLAS_NAME;
texSymbols.filter = "none";

var texPatterns = new JitterObject("jit.gl.texture", "SIGNe");
texPatterns.name = PATTERN_ATLAS_NAME;
...
function build_atlases(signeFolderPath) {
    ...
    // Bind THIS engine's atlas textures to the symbol mesh now that they're
    // populated. We emit the engine-specific names (e.g. SymbolAtlas_JS) via a
    // global send so the patch needs no version logic: only the active engine
    // runs build_atlases, so only its texture names get bound. The receiver
    // (r SIGNe_BindAtlasTextures) feeds the symbol mesh's inlet 0.
    messnamed("SIGNe_BindAtlasTextures", "texture", SYMBOL_ATLAS_NAME, PATTERN_ATLAS_NAME);
...
// Selection-exists flag, broadcast to the patch for the drag-freeze gate.
// -1 = unknown (forces a broadcast on first evaluation). Only emitted on the
// empty<->non-empty transition, and only from redraw_selection_visuals()
// (which fires at selection-change sites, NOT every frame), so this adds no
// per-frame cost and never touches the congested click path.
var _selection_exists = -1;

function _refresh_selection_exists() {
    var registry = _reg();
    var keys = registry.getkeys();
    var anySelected = 0;
    if (keys != null) {
        if (typeof keys === "string") keys = [keys];
        for (var i = 0; i < keys.length; i++) {
            if (registry.get(keys[i] + "::selected") == 1) { anySelected = 1; break; }
        }
    }
    if (anySelected !== _selection_exists) {
        _selection_exists = anySelected;
        messnamed("SIGNe_SelectionExists", anySelected);
    }
}

function redraw_selection_visuals() {
    _refresh_selection_exists();
