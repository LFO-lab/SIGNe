...
inlets = 1;
outlets = 10; // 0-4 for UI, 5-9 directly to jit.gl.multiple
...
// GPU MATRICES
// =========================================================
var matrices_initialized = false;
var matPos, matSym, matPat, matScl, matTil;
var last_total_instances = -1;

function init_matrices() {
    if (!matrices_initialized) {
        matPos = new JitterMatrix(4, "float32", 1); matPos.name = "SIGNe_Pos_Data";
        matSym = new JitterMatrix(4, "float32", 1); matSym.name = "SIGNe_Sym_Data";
        matPat = new JitterMatrix(4, "float32", 1); matPat.name = "SIGNe_Pat_Data";
        matScl = new JitterMatrix(4, "float32", 1); matScl.name = "SIGNe_Scl_Data";
        matTil = new JitterMatrix(3, "float32", 1); matTil.name = "SIGNe_Til_Data";
...
// DRIVEN BY QMETRO 16 (for smooth, throttled updates)
function bang() {
    init_matrices(); 
    ...
    if (needs_recalc) {
        ...
        // Push the matrices to jit.gl.multiple.
        // Inlet 0 (Pos) MUST receive its matrix last to safely trigger the unified draw call.
        outlet(9, "jit_matrix", matTil.name);
        outlet(8, "jit_matrix", matScl.name);
        outlet(7, "jit_matrix", matPat.name);
        outlet(6, "jit_matrix", matSym.name);
        outlet(5, "jit_matrix", matPos.name);
        ...
        if (keys != null) {
            ...
            for (var i = 0; i < keys.length; i++) {
                if (registry.get(keys[i] + "::selected") == 1) {
                    var x = registry.get(keys[i] + "::scale_x") || 1.0, y = registry.get(keys[i] + "::scale_y") || 1.0;
                    activeRatio = (x !== 0) ? (y / x) : 1.0; break;
                }
            }
        }
    }
...
function set_scrubbing(state) {
    isScrubbing = (state === 1);
    if (isScrubbing) { isDraggingMarquee = false; release_group(); outlet(0, "reset"); }
}

function snap(val, quant) {
    if (quant === "free" || quant === 0 || quant === "0" || typeof quant === "undefined") return val;
    var q = parseFloat(quant); if (isNaN(q) || q === 0) return val;
    return Math.round(val * q) / q;
}

function true_wrap(val, max) {
    var v = val - Math.floor(val / max) * max; if (v < 0.0) v += max; return v;
}
...
function get_hit_object(px, py) {
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        for (var j = 0; j < count; j++) {
            ...
            var rad = (rot + gRot) * 2.0 * Math.PI, cosT = Math.cos(-rad), sinT = Math.sin(-rad);
    ...
    if (state === 1) { 
        if (!handledClick) {
            ...
            if (mathHitID == null) {
                ...
                var registry = new Dict("SigneRegistry"); var isSelected = registry.get(mathHitID + "::selected");
                if (isSelected != 1) {
                    var keys = registry.getkeys();
                    if (keys != null) {
                        if (typeof keys === "string") keys = [keys];
                        for (var i = 0; i < keys.length; i++) { registry.set(keys[i] + "::selected", 0); outlet(2, "send", keys[i]); outlet(2, "selected", 0); outlet(2, "selected_via_mouse", 0); }
                    }
                    registry.set(mathHitID + "::selected", 1); outlet(4, mathHitID, 1); outlet(2, "send", mathHitID); outlet(2, "selected", 1);
                    if (linkScale === 1) {
                ...
                got3DAnchor = false; take_centroid_snapshot(registry); outlet(1, "getposition"); draw_selections();
            }
        }
    }
}
...
function screen_mouse(x, y, btn) {
    ...
    if (btn === 1) {
        ...
        } else if (isDraggingGroup || isScalingGroup || isRotatingGroup || isAdjustingOpacityGroup) { outlet(1, "getposition"); }
...
function take_centroid_snapshot(registry) {
    var keys = registry.getkeys(); if (keys == null) return;
    if (typeof keys === "string") keys = [keys];
...
function update_group_positions() {
    var deltaX = c3x - a3x, deltaY = c3y - a3y;
    var registry = new Dict("SigneRegistry"); var keys = registry.getkeys();
    if (keys == null) return; if (typeof keys === "string") keys = [keys];
    for (var i = 0; i < keys.length; i++) {
        var id = keys[i];
        if (registry.get(id + "::selected") == 1) { 
            if (registry.get(id + "::locked") == 1) continue;
            var bx = registry.get(id + "::base_x"); var newX;
            if (snapToTrigger === 1) { var tOff = registry.get(id + "::trigger_offset") || 0.0; newX = snap(bx + tOff + deltaX, quantX) - tOff; } 
    ...
    draw_selections(); mark_dirty(); 
}

function update_group_scale() {
    var deltaX = c3x - a3x; var factorX = 1.0 + deltaX; var factorY = (linkScale === 1) ? factorX : (1.0 + (c3y - a3y));
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        if (registry.get(id + "::selected") == 1) { 
            if (registry.get(id + "::locked") == 1) continue;
            var newX = groupCx + ((registry.get(id + "::base_x") - groupCx) * factorX), newY = groupCy + ((registry.get(id + "::base_y") - groupCy) * factorY);
            var newSx = registry.get(id + "::base_sx") * factorX, newSy = registry.get(id + "::base_sy") * factorY;
            registry.set(id + "::x", newX); registry.set(id + "::y", newY); registry.set(id + "::scale_x", newSx); registry.set(id + "::scale_y", newSy);
            outlet(2, "send", id); outlet(2, "move_x", newX); outlet(2, "move_y", newY); outlet(2, "scale_x", newSx); outlet(2, "scale_y", newSy); outlet(2, "ui_x", newSx); outlet(2, "ui_y", newSy);
        }
    }
    draw_selections(); mark_dirty(); 
}
...
function update_group_opacity() {
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        if (registry.get(id + "::selected") == 1) { 
            ...
            registry.set(id + "::opacity", newOpac); outlet(2, "send", id); outlet(2, "opacity", newOpac);
        }
    }
    draw_selections(); mark_dirty(); 
}
...
function release_selection() {
    if (isScrubbing) return; 
    isDraggingMarquee = false; outlet(0, "reset"); lastViewportInteractionTime = new Date().getTime(); 
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        registry.set(id + "::selected", isSelected); outlet(4, id, (id === leftmostID) ? 1 : 0);
    }
    for (var i = 0; i < keys.length; i++) {
        var id = keys[i]; var isSelected = registry.get(id + "::selected");
        outlet(2, "send", id); outlet(2, "selected", isSelected); outlet(2, "selected_via_mouse", (id === leftmostID) ? 1 : 0);
    }
    draw_selections(); mark_dirty(); 
}
...
// UI INTERACTIONS 
// =========================================================
function remove(id) { var registry = new Dict("SigneRegistry"); if (registry.contains(id)) registry.remove(id); draw_selections(); mark_dirty(); }
function ui_lock(id, state) { var registry = new Dict("SigneRegistry"); if (!registry.contains(id)) return; registry.set(id + "::locked", state); draw_selections(); mark_dirty(); }
function ui_select(target) {
    if (isScrubbing) return;
    if (new Date().getTime() - lastViewportInteractionTime < 500) return;
    var registry = new Dict("SigneRegistry"); if (!registry.contains(target)) return;
    var keys = registry.getkeys();
    if (keys != null) {
        if (typeof keys === "string") keys = [keys];
        for (var i = 0; i < keys.length; i++) { registry.set(keys[i] + "::selected", 0); outlet(2, "send", keys[i]); outlet(2, "selected", 0); outlet(2, "selected_via_mouse", 0); }
    }
    registry.set(target + "::selected", 1); outlet(4, target, 1); outlet(2, "send", target); outlet(2, "selected", 1);
    take_centroid_snapshot(registry);
    if (linkScale === 1) { var x = registry.get(target + "::scale_x") || 1.0, y = registry.get(target + "::scale_y") || 1.0; activeRatio = (x !== 0) ? (y / x) : 1.0; }
    draw_selections(); mark_dirty();
}
function ui_move_x(id, x) {
    var registry = new Dict("SigneRegistry"); if (!registry.contains(id)) return;
    var newX = x;
    if (isHumanX === 1) { if (snapToTrigger === 1) { var tOff = registry.get(id + "::trigger_offset") || 0.0; newX = snap(x + tOff, quantX) - tOff; } else { newX = snap(x, quantX); } }
    registry.set(id + "::x", newX); outlet(2, "send", id); draw_selections(); mark_dirty();
}
function ui_move_y(id, y) {
    var registry = new Dict("SigneRegistry"); if (!registry.contains(id)) return;
    var v = (isHumanY === 1) ? snap(y, quantY) : y; registry.set(id + "::y", v); outlet(2, "send", id); draw_selections(); mark_dirty();
}
function ui_trigger_offset(id, val) {
    var registry = new Dict("SigneRegistry"); if (!registry.contains(id)) return;
    registry.set(id + "::trigger_offset", val); draw_selections(); mark_dirty();
}
function dial_scale_x(id, val, isHuman) {
    ...
    if (linkScale === 1 && humanInteraction === 1) { var newY = val * activeRatio; registry.set(id + "::scale_y", newY); ignoreY = true; outlet(2, "scale_y", newY); outlet(2, "ui_y", newY); ignoreY = false; }
    draw_selections(); mark_dirty();
}
function dial_scale_y(id, val, isHuman) {
    ...
    var humanInteraction = (isHuman !== undefined) ? isHuman : 1;
    if (linkScale === 1 && humanInteraction === 1) { var newX = (activeRatio !== 0) ? (val / activeRatio) : val; registry.set(id + "::scale_x", newX); ignoreX = true; outlet(2, "scale_x", newX); outlet(2, "ui_x", newX); ignoreX = false; }
    draw_selections(); mark_dirty();
}
function ui_scale_x(id, val) { var registry = new Dict("SigneRegistry"); if (!registry.contains(id)) return; registry.set(id + "::scale_x", val); outlet(2, "send", id); draw_selections(); mark_dirty(); }
function ui_scale_y(id, val) { var registry = new Dict("SigneRegistry"); if (!registry.contains(id)) return; registry.set(id + "::scale_y", val); outlet(2, "send", id); draw_selections(); mark_dirty(); }
function ui_rotate(id, val) { var registry = new Dict("SigneRegistry"); if (!registry.contains(id)) return; registry.set(id + "::rotation", val); outlet(2, "send", id); draw_selections(); mark_dirty(); }
function ui_opacity(id, val) { var registry = new Dict("SigneRegistry"); if (!registry.contains(id)) return; registry.set(id + "::opacity", val); outlet(2, "send", id); draw_selections(); mark_dirty(); }
function ui_count(id, val) { var registry = new Dict("SigneRegistry"); if (!registry.contains(id)) return; registry.set(id + "::count", val); outlet(2, "send", id); draw_selections(); mark_dirty(); }
function ui_spacing(id, val) { var registry = new Dict("SigneRegistry"); if (!registry.contains(id)) return; var v = (isHumanSpacing === 1) ? snap(val, quantSpacing) : val; registry.set(id + "::spacing", v); outlet(2, "send", id); draw_selections(); mark_dirty(); }
function ui_group_rot(id, val) { var registry = new Dict("SigneRegistry"); if (!registry.contains(id)) return; registry.set(id + "::group_rot", val); outlet(2, "send", id); draw_selections(); mark_dirty(); }
function ui_bounds_x(id, val) { var registry = new Dict("SigneRegistry"); if (!registry.contains(id)) return; registry.set(id + "::bounds_x", val); draw_selections(); mark_dirty(); }
function ui_bounds_y(id, val) { var registry = new Dict("SigneRegistry"); if (!registry.contains(id)) return; registry.set(id + "::bounds_y", val); draw_selections(); mark_dirty(); }

// =========================================================
// CAMERA TRACKING & TRANSPORT
// =========================================================
function set_pinned(id, state) { var registry = new Dict("SigneRegistry"); if (registry.contains(id)) registry.set(id + "::pinned", state); }
...
function move_to_transport(id) {
    ...
    var registry = new Dict("SigneRegistry"); if (!registry.contains(id)) return;
    if (snapToTrigger === 1) { var tOff = registry.get(id + "::trigger_offset") || 0.0; v = snap(bars, quantX) - tOff; } else { v = snap(bars, quantX); }
    registry.set(id + "::x", v); outlet(2, "send", id); outlet(2, "move_x", v); draw_selections(); mark_dirty();
}
...
function group_prop_symbol(propName, filename) {
    ...
    var manifest = new Dict("AtlasManifest"); var searchArray = isPattern ? manifest.get("patterns") : manifest.get("symbols");
    var targetIdx = 0.0;
    if (searchArray != null) {
...
function update_math() {
    ...
    for (var i = 0; i < keys.length; i++) { total_instances += (parseInt(registry.get(keys[i] + "::count")) || 1); }

    if (total_instances !== last_total_instances) {
        matPos.dim = total_instances; matSym.dim = total_instances; matPat.dim = total_instances; matScl.dim = total_instances; matTil.dim = total_instances;
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        var rot = parseFloat(registry.get(id + "::rotation")) || 0.0, rotRadians = rot * 2.0 * Math.PI;

        var sx = parseFloat(registry.get(id + "::scale_x")); if (isNaN(sx)) sx = 1.0;
        var sy = parseFloat(registry.get(id + "::scale_y")); if (isNaN(sy)) sy = 1.0;
        var symIdx = parseFloat(registry.get(id + "::sym_idx")) || 0.0, patIdx = parseFloat(registry.get(id + "::pat_idx")) || 0.0;
        ...
        var count = parseInt(registry.get(id + "::count")) || 1, spacing = parseFloat(registry.get(id + "::spacing")) || 0.0;
        var gRot = parseFloat(registry.get(id + "::group_rot")) || 0.0, gCos = Math.cos(-gRot * 2.0 * Math.PI), gSin = Math.sin(-gRot * 2.0 * Math.PI);
